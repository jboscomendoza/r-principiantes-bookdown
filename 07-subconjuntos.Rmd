...
Notarás que el resultado que obtenemos no es un data frame, sino un vector. Esta es una característica distintiva de este método y veremos la explicación de este comportamiento más adelante en este capítulo.

Este método funciona de la misma manera para listas.
```{r, echo=TRUE, error=TRUE}
# Nombre sin comillas
mi_lista$uno
...
## El corchete doble `[[]]`
Cuando usamos corchetes para obtener subconjuntos de un data frame, obtenemos resultados diferentes que al usar el signo `$`, aunque estemos pidiendo los mismos elementos. Con corchetes obtenemos un data frame mientras que con el signo `$` nos es devuelto un vector.
```{r, echo = TRUE}
class(mi_df["nombre"])
...
Para entender porqué ocurre esto, recordemos que **un data frame está formado por vectores**. Estos vectores, aunque estén contenidos dentro de un data frame, siguen conservan todas las características de un vector y es posible extraerlos como tales.
