...
Como podrás ver, las estructuras de datos más sencillas, (unidimensionales, homogeneas) pueden ser coercionadas a otras más complejas (multidimensionales, heterogeneas), pero la operación inversa casi nunca es posible.
...
Nota que `as.vector()` no devolvió un error o una advertencia a pesar de que no tuvo éxito para al coercionar un data frame o una lista a un vector. Esto es importante, pues no puedes confiar que `as.vector()` tuvo éxito porque corrió sin errores, es necesaria una verificación adicional.
...
Nota que para los vectores y matrices, 
